//! Shared tool name mappings between OpenClaw and Skipper.
//!
//! These mappings are used by both the migration engine and the skill system
//! to normalize OpenClaw tool names into Skipper equivalents.

/// Map an OpenClaw tool name to its Skipper equivalent.
///
/// Returns `None` if the name has no known mapping (may already be
/// an Skipper tool name — check with [`is_known_skipper_tool`]).
...
/// Check if a tool name is a known Skipper built-in tool.
pub fn is_known_skipper_tool(name: &str) -> bool {
    matches!(
        name,
        "file_read"
            | "file_write"
...
mod tests {
    ...
    fn test_is_known_skipper_tool() {
        ...
        for tool in &known {
            assert!(is_known_skipper_tool(tool), "Expected {tool} to be known");
        }

        // Unknown
        assert!(!is_known_skipper_tool("unknown"));
        assert!(!is_known_skipper_tool("Read"));
        assert!(!is_known_skipper_tool("Bash"));
    }
}
