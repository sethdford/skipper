...
//!
//! Boots the Skipper kernel, binds to a random localhost port, and runs the
//! API server on a background thread with its own tokio runtime.

use skipper_api::server::build_router;
...
impl ServerHandle {
    ...
    pub fn shutdown(mut self) {
        ...
        info!("Skipper embedded server stopped");
    }
}
...
pub fn start_server() -> Result<ServerHandle, Box<dyn std::error::Error>> {
    ...
    let kernel = SkipperKernel::boot(None)?;
    ...
    info!("Skipper embedded server bound to http://127.0.0.1:{port}");

    let (shutdown_tx, shutdown_rx) = watch::channel(false);
    let kernel_clone = kernel.clone();

    let server_thread = std::thread::Builder::new()
        .name("skipper-server".into())
        .spawn(move || {
...
async fn run_embedded_server(
    ...
    info!("Skipper embedded server listening on http://{listen_addr}");
