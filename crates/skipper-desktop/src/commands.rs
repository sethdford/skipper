...
use skipper_kernel::config::skipper_home;
...
/// `~/.skipper/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = skipper_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.skipper/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = skipper_home().join("skills");
...
/// Open the Skipper config directory (`~/.skipper/`) in the OS file manager.
#[tauri::command]
pub fn open_config_dir() -> Result<(), String> {
    let dir = skipper_home();
    std::fs::create_dir_all(&dir).map_err(|e| format!("Failed to create config dir: {e}"))?;
    open::that(&dir).map_err(|e| format!("Failed to open directory: {e}"))
}

/// Open the Skipper logs directory (`~/.skipper/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = skipper_home().join("logs");
