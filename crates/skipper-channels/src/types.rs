...
pub struct ChannelUser {
    ...
    /// Optional mapping to an Skipper user identity.
    pub skipper_user: Option<String>,
}
...
// Re-export policy/format types from skipper-types for convenience.
pub use skipper_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the Skipper kernel by converting
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = skipper_types::truncate_str(remaining, max_len).len();
