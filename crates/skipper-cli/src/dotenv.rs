//! Minimal `.env` file loader/saver for `~/.skipper/.env`.
//!
...
/// Return the path to `~/.skipper/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".skipper").join(".env"))
}

/// Load `~/.skipper/.env` and `~/.skipper/secrets.env` into `std::env`.
///
...
/// Return the path to `~/.skipper/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".skipper").join("secrets.env"))
}
...
/// List key names (without values) from `~/.skipper/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content =
        String::from("# Skipper environment — managed by `skipper config set-key`\n");
