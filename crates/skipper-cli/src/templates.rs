...
/// 2. `~/.skipper/agents/` (installed templates)
/// 3. `SKIPPER_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs::home_dir() {
        let agents = home.join(".skipper").join("agents");
    ...
    if let Ok(env_dir) = std::env::var("SKIPPER_AGENTS_DIR") {
