//! MCP (Model Context Protocol) server for Skipper.
//!
//! Exposes running agents as MCP tools over JSON-RPC 2.0 stdio.
//! Each agent becomes a callable tool named `skipper_agent_{name}`.
//!
...
impl McpBackend {
    ...
    /// Find agent ID by tool name (strip `skipper_agent_` prefix, match by name).
    fn resolve_tool_agent(&self, tool_name: &str) -> Option<String> {
        let agent_name = tool_name.strip_prefix("skipper_agent_")?.replace('_', "-");
        ...
        let agent_name_underscore = tool_name.strip_prefix("skipper_agent_")?;
        for (id, name, _) in &agents {
        ...
        "tools/list" => {
            ...
            let tools: Vec<Value> = agents
                ...
                .map(|(_, name, description)| {
                    let tool_name = format!("skipper_agent_{}", name.replace('-', "_"));
                    let desc = if description.is_empty() {
                        format!("Send a message to Skipper agent '{name}'")
                    } else {
                        description.clone()
                    };
                    json!({
...
mod tests {
    ...
    fn test_handle_initialize() {
        ...
        assert_eq!(resp["result"]["serverInfo"]["name"], "skipper");
    }
