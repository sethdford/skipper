//! Standalone chat TUI for `skipper chat`.
//!
...
enum Backend {
    ...
    InProcess { kernel: Arc<SkipperKernel> },
    None,
}
...
impl StandaloneChat {
    ...
    fn handle_stream_done(
        &mut self,
        result: Result<skipper_runtime::agent_loop::AgentLoopResult, String>,
    ) {
    ...
    fn handle_kernel_ready(&mut self, kernel: Arc<SkipperKernel>) {
        self.booting = false;
    ...
    fn resolve_daemon_agent(&mut self, base_url: &str, agent_name: Option<&str>) {
        ...
        match template {
            ...
            None => {
                self.boot_error =
                    Some("No agent templates found. Run `skipper init`.".to_string());
            }
        }
    }
