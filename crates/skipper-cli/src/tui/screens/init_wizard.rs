...
//!
//! Launched by `skipper init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use skipper_runtime::model_catalog::ModelCatalog;
use skipper_types::model_catalog::ModelTier;
...
struct State {
    ...
    openclaw_scan: Option<skipper_migrate::openclaw::ScanResult>,
    migration_report: Option<skipper_migrate::report::MigrationReport>,
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<skipper_migrate::report::MigrationReport, String>>();
...
fn handle_migration_key(
    state: &mut State,
    code: KeyCode,
    migrate_tx: &std::sync::mpsc::Sender<Result<skipper_migrate::report::MigrationReport, String>>,
) {
        ...
        MigrationPhase::Offer => match code {
            ...
            KeyCode::Enter => {
                ...
                if yes {
                    ...
                    let target_dir = home.join(".skipper");
                    let tx = migrate_tx.clone();
                    std::thread::spawn(move || {
                        let options = skipper_migrate::MigrateOptions {
                            source: skipper_migrate::MigrateSource::OpenClaw,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result =
                            skipper_migrate::run_migration(&options).map_err(|e| format!("{e}"));
...
fn save_config(state: &mut State) {
    ...
    let _ = std::fs::create_dir_all(skipper_dir.join("agents"));
    let _ = std::fs::create_dir_all(skipper_dir.join("data"));
    crate::restrict_dir_permissions(&skipper_dir);
...
/// Check if the `skipper-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "Skipper Init  Step X of 7"
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use skipper_migrate::report::ItemKind;
...
fn draw_complete(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  How do you want to use Skipper?",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),
