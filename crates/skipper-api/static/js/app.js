// Skipper App — Alpine.js init, hash router, global store
"use strict";
...
function toolIcon(toolName) {
  ...
  if (n.indexOf("file_") === 0 || n.indexOf("directory_") === 0)
    return (
      "<svg " +
      s +
      '><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/></svg>'
    );
  // Web/fetch
  if (n.indexOf("web_") === 0 || n.indexOf("link_") === 0)
    return (
      "<svg " +
      s +
      '><circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15 15 0 0 1 4 10 15 15 0 0 1-4 10 15 15 0 0 1-4-10 15 15 0 0 1 4-10z"/></svg>'
    );
  // Shell/exec
  ...
  if (n.indexOf("container_") === 0 || n.indexOf("docker_") === 0)
    return (
      "<svg " +
      s +
      '><path d="M22 12H2"/><path d="M5.45 5.11L2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg>'
    );
  // Image/media
  ...
  if (n.indexOf("hand_") === 0)
    return (
      "<svg " +
      s +
      '><path d="M18 11V6a2 2 0 0 0-2-2 2 2 0 0 0-2 2"/><path d="M14 10V4a2 2 0 0 0-2-2 2 2 0 0 0-2 2v6"/><path d="M10 10.5V6a2 2 0 0 0-2-2 2 2 0 0 0-2 2v8"/><path d="M18 8a2 2 0 1 1 4 0v6a8 8 0 0 1-8 8h-2c-2.8 0-4.5-.9-5.7-2.4L3.4 16a2 2 0 0 1 3.2-2.4L8 15"/></svg>'
    );
  // Task/collab
...
document.addEventListener("alpine:init", function () {
  ...
  var savedKey = localStorage.getItem("skipper-api-key");
  if (savedKey) SkipperAPI.setAuthToken(savedKey);
  ...
  Alpine.store("app", {
    ...
    agentCount: 0,
    pendingAgent: null,
    focusMode: localStorage.getItem("skipper-focus") === "true",
    showOnboarding: false,
    showAuthPrompt: false,

    toggleFocusMode() {
      this.focusMode = !this.focusMode;
      localStorage.setItem("skipper-focus", this.focusMode);
    },

    async refreshAgents() {
      try {
        var agents = await SkipperAPI.get("/api/agents");
    ...
    async checkStatus() {
      try {
        var s = await SkipperAPI.get("/api/status");
        this.connected = true;
        this.booting = false;
        this.lastError = "";
        ...
        console.warn("[Skipper] Status check failed:", e.message);
      }
    },

    async checkOnboarding() {
      if (localStorage.getItem("skipper-onboarded")) return;
      try {
        var config = await SkipperAPI.get("/api/config");
    ...
    dismissOnboarding() {
      ...
      localStorage.setItem("skipper-onboarded", "true");
    },
    ...
    async checkAuth() {
      try {
        ...
        await SkipperAPI.get("/api/tools");
        this.showAuthPrompt = false;
      } catch (e) {
        if (
          e.message &&
          (e.message.indexOf("Not authorized") >= 0 ||
            e.message.indexOf("401") >= 0 ||
            e.message.indexOf("Missing Authorization") >= 0 ||
            e.message.indexOf("Unauthorized") >= 0)
        ) {
          // Only show prompt if we don't already have a saved key
          var saved = localStorage.getItem("skipper-api-key");
          if (saved) {
            // Saved key might be stale — clear it and show prompt
            SkipperAPI.setAuthToken("");
            localStorage.removeItem("skipper-api-key");
          }
    ...
    submitApiKey(key) {
      ...
      SkipperAPI.setAuthToken(key.trim());
      localStorage.setItem("skipper-api-key", key.trim());
      this.showAuthPrompt = false;
      this.refreshAgents();
    },
...
function app() {
  return {
    page: "agents",
    themeMode: localStorage.getItem("skipper-theme-mode") || "system",
    theme: (() => {
      var mode = localStorage.getItem("skipper-theme-mode") || "system";
      if (mode === "system")
    ...
    sidebarCollapsed: localStorage.getItem("skipper-sidebar") === "collapsed",
    ...
    init() {
      ...
      SkipperAPI.onConnectionChange(function (state) {
    ...
    setTheme(mode) {
      this.themeMode = mode;
      localStorage.setItem("skipper-theme-mode", mode);
      if (mode === "system") {
    ...
    async pollStatus() {
      ...
      this.wsConnected = SkipperAPI.isWsConnected();
    },
  };
}
