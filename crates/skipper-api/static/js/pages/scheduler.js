// Skipper Scheduler Page — Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadJobs() {
      var data = await SkipperAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await SkipperAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        SkipperToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        SkipperToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await SkipperAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true };
        SkipperToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        SkipperToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await SkipperAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        SkipperToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        SkipperToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      SkipperToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await SkipperAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          SkipperToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          SkipperToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await SkipperAPI.post('/api/schedules/' + job.id + '/run', {});
        if (result.status === 'completed') {
          SkipperToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          SkipperToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        SkipperToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await SkipperAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        SkipperToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        SkipperToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      SkipperToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await SkipperAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          SkipperToast.success('Trigger deleted');
        } catch(e) {
          SkipperToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
