// Skipper Overview Dashboard — Landing page with system stats + provider status
'use strict';
...
function overviewPage() {
  return {
    ...
    async loadHealth() {
      try {
        this.health = await SkipperAPI.get('/api/health');
      } catch(e) { this.health = { status: 'unreachable' }; }
    },

    async loadStatus() {
      try {
        this.status = await SkipperAPI.get('/api/status');
    ...
    async loadAudit() {
      try {
        var data = await SkipperAPI.get('/api/audit/recent?n=8');
