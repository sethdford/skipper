// Skipper Hands Page — curated autonomous capability packages
'use strict';
...
function handsPage() {
  return {
    ...
    async loadData() {
      ...
      this.loadError = '';
      try {
        var data = await SkipperAPI.get('/api/hands');
    ...
    async loadActive() {
      ...
      try {
        var data = await SkipperAPI.get('/api/hands/active');
    ...
    async showDetail(handId) {
      try {
        var data = await SkipperAPI.get('/api/hands/' + handId);
    ...
    async installDeps() {
      ...
      try {
        var data = await SkipperAPI.post('/api/hands/' + handId + '/install-deps', {});
    ...
    async recheckDeps() {
      ...
      try {
        var data = await SkipperAPI.post('/api/hands/' + this.setupWizard.id + '/check-deps', {});
        if (data.requirements && this.setupWizard.requirements) {
          for (var i = 0; i < this.setupWizard.requirements.length; i++) {
            var existing = this.setupWizard.requirements[i];
            for (var j = 0; j < data.requirements.length; j++) {
    ...
    async launchHand() {
      ...
      try {
        var data = await SkipperAPI.post('/api/hands/' + handId + '/activate', { config: config });
    ...
    async pauseHand(inst) {
      try {
        await SkipperAPI.post('/api/hands/instances/' + inst.instance_id + '/pause', {});
        inst.status = 'Paused';
      } catch(e) {
    ...
    async resumeHand(inst) {
      try {
        await SkipperAPI.post('/api/hands/instances/' + inst.instance_id + '/resume', {});
        inst.status = 'Active';
      } catch(e) {
    ...
    async deactivate(inst) {
      ...
      SkipperToast.confirm('Deactivate Hand', 'Deactivate hand "' + handName + '"? This will kill its agent.', async function() {
        try {
          await SkipperAPI.delete('/api/hands/instances/' + inst.instance_id);
          self.instances = self.instances.filter(function(i) { return i.instance_id !== inst.instance_id; });
          SkipperToast.success('Hand deactivated.');
        } catch(e) {
          SkipperToast.error('Deactivation failed: ' + (e.message || 'unknown error'));
        }
      });
    },
    ...
    async loadStats(inst) {
      try {
        var data = await SkipperAPI.get('/api/hands/instances/' + inst.instance_id + '/stats');
    ...
    async refreshBrowserView() {
      ...
      try {
        var data = await SkipperAPI.get('/api/hands/instances/' + id + '/browser');
        if (data.active) {
