// Skipper Skills Page — OpenClaw/ClawHub ecosystem + local skills + MCP servers
'use strict';
...
function skillsPage() {
  return {
    ...
    async loadSkills() {
      ...
      this.loadError = '';
      try {
        var data = await SkipperAPI.get('/api/skills');
    ...
    async searchClawHub() {
      ...
      this.clawhubError = '';
      try {
        var data = await SkipperAPI.get('/api/clawhub/search?q=' + encodeURIComponent(this.clawhubSearch.trim()) + '&limit=20');
    ...
    async browseClawHub(sort) {
      ...
      try {
        var data = await SkipperAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20');
    ...
    async loadMoreClawHub() {
      ...
      this.clawhubLoading = true;
      try {
        var data = await SkipperAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20&cursor=' + encodeURIComponent(this.clawhubNextCursor));
    ...
    async showSkillDetail(slug) {
      ...
      try {
        var data = await SkipperAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug));
        this.skillDetail = data;
      } catch(e) {
        SkipperToast.error('Failed to load skill details');
      }
    ...
    async installFromClawHub(slug) {
      ...
      this.installResult = null;
      try {
        var data = await SkipperAPI.post('/api/clawhub/install', { slug: slug });
        this.installResult = data;
        if (data.warnings && data.warnings.length > 0) {
          SkipperToast.success('Skill "' + data.name + '" installed with ' + data.warnings.length + ' warning(s)');
        } else {
          SkipperToast.success('Skill "' + data.name + '" installed successfully');
        }
        ...
        if (msg.includes('already_installed')) {
          SkipperToast.error('Skill is already installed');
        } else if (msg.includes('SecurityBlocked')) {
          SkipperToast.error('Skill blocked by security scan');
        } else {
          SkipperToast.error('Install failed: ' + msg);
        }
      }
    ...
    uninstallSkill: function(name) {
      var self = this;
      SkipperToast.confirm('Uninstall Skill', 'Uninstall skill "' + name + '"? This cannot be undone.', async function() {
        try {
          await SkipperAPI.post('/api/skills/uninstall', { name: name });
          SkipperToast.success('Skill "' + name + '" uninstalled');
          await self.loadSkills();
        } catch(e) {
          SkipperToast.error('Failed to uninstall skill: ' + e.message);
        }
      });
    },
    ...
    async createDemoSkill(skill) {
      try {
        await SkipperAPI.post('/api/skills/create', {
          name: skill.name,
          description: skill.description,
          runtime: 'prompt_only',
          prompt_context: skill.prompt_context || skill.description
        });
        SkipperToast.success('Skill "' + skill.name + '" created');
        this.tab = 'installed';
        await this.loadSkills();
      } catch(e) {
        SkipperToast.error('Failed to create skill: ' + e.message);
      }
    },
    ...
    async loadMcpServers() {
      ...
      try {
        var data = await SkipperAPI.get('/api/mcp/servers');
