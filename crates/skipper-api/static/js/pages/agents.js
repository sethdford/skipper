...
function agentsPage() {
  return {
    ...
    async loadTemplates() {
      ...
      try {
        var results = await Promise.all([
          ...
          SkipperAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    killAgent(agent) {
      var self = this;
      SkipperToast.confirm('Stop Agent', 'Stop agent "' + agent.name + '"? The agent will be shut down.', async function() {
        try {
    ...
    killAllAgents() {
      ...
      SkipperToast.confirm('Stop All Agents', 'Stop ' + list.length + ' agent(s)? All agents will be shut down.', async function() {
        var errors = [];
        ...
        await Alpine.store('app').refreshAgents();
        if (errors.length) {
          SkipperToast.error('Some agents failed to stop: ' + errors.join(', '));
        } else {
    ...
    async setMode(agent, mode) {
      try {
        await SkipperAPI.put('/api/agents/' + agent.id + '/mode', { mode: mode });
        agent.mode = mode;
      ...
      try {
        ...
        if (res.agent_id) {
          ...
          if (Object.keys(patchBody).length) {
            SkipperAPI.patch('/api/agents/' + res.agent_id + '/config', patchBody).catch(function(e) { console.warn('Post-spawn config patch failed:', e.message); });
          }
          if (this.soulContent.trim()) {
            SkipperAPI.put('/api/agents/' + res.agent_id + '/files/SOUL.md', { content: '# Soul\n' + this.soulContent }).catch(function(e) { console.warn('SOUL.md write failed:', e.message); });
          }
    ...
    async loadAgentFiles() {
      ...
      try {
        var data = await SkipperAPI.get('/api/agents/' + this.detailAgent.id + '/files');
    ...
    async openFile(file) {
      ...
      try {
        var data = await SkipperAPI.get('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(file.name));
        this.editingFile = file.name;
    ...
    async saveFile() {
      ...
      try {
        await SkipperAPI.put('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(this.editingFile), { content: this.fileContent });
    ...
    async saveConfig() {
      ...
      try {
        await SkipperAPI.patch('/api/agents/' + this.detailAgent.id + '/config', this.configForm);
    ...
    async cloneAgent(agent) {
      ...
      try {
        var res = await SkipperAPI.post('/api/agents/' + agent.id + '/clone', { new_name: newName });
        if (res.agent_id) {
    ...
    async spawnFromTemplate(name) {
      try {
        var data = await SkipperAPI.get('/api/templates/' + encodeURIComponent(name));
        if (data.manifest_toml) {
          var res = await SkipperAPI.post('/api/agents', { manifest_toml: data.manifest_toml });
          if (res.agent_id) {
            SkipperToast.success('Agent "' + (res.name || name) + '" spawned from template');
            await Alpine.store('app').refreshAgents();
