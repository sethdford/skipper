// Skipper Settings Page — Provider Hub, Model Catalog, Config, Tools + Security, Network, Migration tabs
'use strict';
  ...
  return {
    ...
    configurableFeatures: [
      ...
      },
      {
        ...
        configHint: 'Set api_key in ~/.skipper/config.toml for remote access. Empty = localhost only.',
        valueKey: 'auth'
      }
    ],
    ...
    async loadSysInfo() {
      try {
        var ver = await SkipperAPI.get('/api/version');
        var status = await SkipperAPI.get('/api/status');
        this.sysInfo = {
          version: ver.version || '-',
    ...
    async loadUsage() {
      try {
        var data = await SkipperAPI.get('/api/usage');
    ...
    async loadTools() {
      try {
        var data = await SkipperAPI.get('/api/tools');
    ...
    async loadConfig() {
      try {
        this.config = await SkipperAPI.get('/api/config');
      } catch(e) { this.config = {}; }
    },

    async loadProviders() {
      try {
        var data = await SkipperAPI.get('/api/providers');
    ...
    async loadModels() {
      try {
        var data = await SkipperAPI.get('/api/models');
    ...
    async addCustomModel() {
      ...
      try {
        await SkipperAPI.post('/api/models/custom', {
          id: id,
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          SkipperAPI.get('/api/config/schema').catch(function() { return {}; }),
          SkipperAPI.get('/api/config')
        ]);
    ...
    async saveConfigField(section, field, value) {
      ...
      try {
        await SkipperAPI.post('/api/config/set', { path: key, value: value });
        this.configDirty[key] = false;
        SkipperToast.success('Saved ' + key);
      } catch(e) {
        SkipperToast.error('Failed to save: ' + e.message);
      }
    ...
    async saveProviderKey(provider) {
      ...
      if (!key || !key.trim()) { SkipperToast.error('Please enter an API key'); return; }
      try {
        await SkipperAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key.trim() });
        SkipperToast.success('API key saved for ' + provider.display_name);
        this.providerKeyInputs[provider.id] = '';
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        SkipperToast.error('Failed to save key: ' + e.message);
      }
    },

    async removeProviderKey(provider) {
      try {
        await SkipperAPI.del('/api/providers/' + encodeURIComponent(provider.id) + '/key');
        SkipperToast.success('API key removed for ' + provider.display_name);
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        SkipperToast.error('Failed to remove key: ' + e.message);
      }
    },
    ...
    async startCopilotOAuth() {
      ...
      try {
        var resp = await SkipperAPI.post('/api/providers/github-copilot/oauth/start', {});
        ...
        SkipperToast.error('Failed to start Copilot login: ' + e.message);
    ...
    pollCopilotOAuth() {
      ...
      setTimeout(async function() {
        ...
        try {
          var resp = await SkipperAPI.get('/api/providers/github-copilot/oauth/poll/' + self.copilotOAuth.pollId);
          if (resp.status === 'complete') {
            SkipperToast.success('GitHub Copilot authenticated successfully!');
            ...
            SkipperToast.error('Device code expired. Please try again.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else if (resp.status === 'denied') {
            SkipperToast.error('Access denied by user.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else {
            SkipperToast.error('OAuth error: ' + (resp.error || resp.status));
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          }
        } catch(e) {
          SkipperToast.error('Poll error: ' + e.message);
          self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
        }
    ...
    async testProvider(provider) {
      ...
      try {
        var result = await SkipperAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/test', {});
        this.providerTestResults[provider.id] = result;
        if (result.status === 'ok') {
          SkipperToast.success(provider.display_name + ' connected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          SkipperToast.error(provider.display_name + ': ' + (result.error || 'Connection failed'));
        }
      } catch(e) {
        this.providerTestResults[provider.id] = { status: 'error', error: e.message };
        SkipperToast.error('Test failed: ' + e.message);
      }
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { SkipperToast.error('Please enter a base URL'); return; }
      url = url.trim();
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        SkipperToast.error('URL must start with http:// or https://'); return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await SkipperAPI.put('/api/providers/' + encodeURIComponent(provider.id) + '/url', { base_url: url });
        if (result.reachable) {
          SkipperToast.success(provider.display_name + ' URL saved &mdash; reachable (' + (result.latency_ms || '?') + 'ms)');
        } else {
          SkipperToast.warning(provider.display_name + ' URL saved but not reachable');
        }
        await this.loadProviders();
      } catch(e) {
        SkipperToast.error('Failed to save URL: ' + e.message);
      }
    ...
    async loadSecurity() {
      ...
      try {
        this.securityData = await SkipperAPI.get('/api/security');
      } catch(e) {
    ...
    async verifyAuditChain() {
      ...
      try {
        var res = await SkipperAPI.get('/api/audit/verify');
    ...
    async loadPeers() {
      ...
      this.peersLoadError = '';
      try {
        var data = await SkipperAPI.get('/api/peers');
    ...
    async autoDetect() {
      this.detecting = true;
      try {
        var data = await SkipperAPI.get('/api/migrate/detect');
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await SkipperAPI.post('/api/migrate/scan', { path: this.sourcePath });
        if (data.error) {
          SkipperToast.error('Scan error: ' + data.error);
          this.scanning = false;
          return;
        }
        ...
        SkipperToast.error('Scan failed: ' + e.message);
      }
      this.scanning = false;
    },
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        if (!target) target = '';
        var data = await SkipperAPI.post('/api/migrate', {
          source: 'openclaw',
