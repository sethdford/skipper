// Skipper Setup Wizard — First-run guided setup (Provider + Agent + Channel)
'use strict';
...
function wizardPage() {
  return {
    ...
    async saveKey() {
      ...
      if (!key) {
        SkipperToast.error('Please enter an API key');
        return;
      }
      this.savingKey = true;
      try {
        await SkipperAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key });
        this.apiKeyInput = '';
        this.keySaved = true;
    ...
    async createAgent() {
      ...
      if (!name) {
        SkipperToast.error('Please enter a name for your agent');
        return;
      }
      ...
      try {
        ...
        if (res.agent_id) {
          ...
          SkipperToast.success('Agent "' + (res.name || name) + '" created');
          await Alpine.store('app').refreshAgents();
        } else {
          SkipperToast.error('Failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        SkipperToast.error('Failed to create agent: ' + e.message);
      }
    ...
    async configureChannel() {
      ...
      if (!token) {
        SkipperToast.error('Please enter the ' + ch.token_label);
        return;
      }
      ...
      try {
        ...
        fields.token = token;
        await SkipperAPI.post('/api/channels/' + ch.name + '/configure', { fields: fields });
        this.channelConfigured = true;
        this.setupSummary.channel = ch.display_name;
        SkipperToast.success(ch.display_name + ' configured and activated.');
      } catch(e) {
