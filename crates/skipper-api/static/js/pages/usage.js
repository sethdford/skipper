// Skipper Analytics Page — Full usage analytics with per-model and per-agent breakdowns
...
function analyticsPage() {
  return {
    ...
    async loadSummary() {
      try {
        this.summary = await SkipperAPI.get("/api/usage/summary");
      } catch (e) {
        this.summary = {
    ...
    async loadByModel() {
      try {
        var data = await SkipperAPI.get("/api/usage/by-model");
    ...
    async loadDailyCosts() {
      try {
        var data = await SkipperAPI.get("/api/usage/daily");
    ...
    _extractProvider(modelName) {
      ...
      if (
        lower.indexOf("claude") !== -1 ||
        lower.indexOf("haiku") !== -1 ||
        lower.indexOf("sonnet") !== -1 ||
        lower.indexOf("opus") !== -1
      )
        return "Anthropic";
      if (lower.indexOf("gemini") !== -1 || lower.indexOf("gemma") !== -1)
        return "Google";
      if (
        lower.indexOf("gpt") !== -1 ||
        lower.indexOf("o1") !== -1 ||
        ...
        return "OpenAI";
      if (
        lower.indexOf("llama") !== -1 ||
        lower.indexOf("mixtral") !== -1 ||
        lower.indexOf("groq") !== -1
      )
        return "Groq";
      if (lower.indexOf("deepseek") !== -1) return "DeepSeek";
      if (lower.indexOf("mistral") !== -1) return "Mistral";
      if (lower.indexOf("command") !== -1 || lower.indexOf("cohere") !== -1)
        return "Cohere";
    ...
    barChartData() {
      ...
      for (var i = 0; i < days.length; i++) {
        ...
        var heightPct = Math.max(
          2,
          Math.round((days[i].cost_usd / maxCost) * 120),
        );
        result.push({
          date: days[i].date,
          dayName: dayName,
    ...
    costBarWidth(m) {
      return (
        Math.max(
          2,
          Math.round(((m.total_cost_usd || 0) / this.maxModelCost()) * 100),
        ) + "%"
      );
    },
    ...
    modelTier(modelName) {
      ...
      if (
        ...
        lower.indexOf("o1") !== -1 ||
        lower.indexOf("o3") !== -1 ||
        lower.indexOf("deepseek-r1") !== -1
      )
        return "frontier";
      if (
        lower.indexOf("sonnet") !== -1 ||
        lower.indexOf("gpt-4") !== -1 ||
        lower.indexOf("gemini-2.5") !== -1 ||
        lower.indexOf("gemini-1.5-pro") !== -1
      )
        return "smart";
      if (
        lower.indexOf("haiku") !== -1 ||
        lower.indexOf("gpt-3.5") !== -1 ||
