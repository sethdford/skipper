// Skipper Logs Page — Real-time log viewer (SSE streaming + polling fallback) + Audit Trail tab
'use strict';
...
function logsPage() {
  return {
    ...
    async fetchLogs() {
      ...
      try {
        var data = await SkipperAPI.get('/api/audit/recent?n=200');
    ...
    exportLogs: function() {
      ...
      a.href = url;
      a.download = 'skipper-logs-' + new Date().toISOString().slice(0, 10) + '.txt';
      a.click();
    ...
    async verifyChain() {
      try {
        var data = await SkipperAPI.get('/api/audit/verify');
        this.chainValid = data.valid === true;
        if (this.chainValid) {
          SkipperToast.success('Audit chain verified — ' + (data.entries || 0) + ' entries valid');
        } else {
          SkipperToast.error('Audit chain broken!');
        }
      } catch(e) {
        this.chainValid = false;
        SkipperToast.error('Chain verification failed: ' + e.message);
      }
    },
