// Skipper Channels Page — OpenClaw-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async loadChannels() {
      ...
      this.loadError = '';
      try {
        var data = await SkipperAPI.get('/api/channels');
    ...
    async startQR() {
      ...
      try {
        var result = await SkipperAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          SkipperToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await SkipperAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            SkipperToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await SkipperAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await SkipperAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            SkipperToast.success(this.setupModal.display_name + ' activated!');
          } else {
            SkipperToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          SkipperToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
        SkipperToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async removeChannel() {
      ...
      var self = this;
      SkipperToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await SkipperAPI.delete('/api/channels/' + name + '/configure');
          SkipperToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await SkipperAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          SkipperToast.success(result.message);
        } else {
          SkipperToast.error(result.message);
        }
      } catch(e) {
        SkipperToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        SkipperToast.success('Copied to clipboard');
      } catch(e) {
        SkipperToast.error('Copy failed');
      }
    },
