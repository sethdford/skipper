...
//!
//! Allows any OpenAI-compatible client library to talk to Skipper agents.
//! The `model` field resolves to an agent (by name, UUID, or `skipper:<name>`),
...
use skipper_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "skipper:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("skipper:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("skipper:{}", e.name),
            object: "model",
            created,
            owned_by: "skipper".to_string(),
        })
        .collect();
