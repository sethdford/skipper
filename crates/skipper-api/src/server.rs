//! Skipper daemon server — boots the kernel and serves the HTTP API.
...
/// Daemon info written to `~/.skipper/daemon.json` so the CLI can find us.
...
///
/// This is extracted from `run_daemon()` so that embedders (e.g. skipper-desktop)
...
/// Start the Skipper daemon: boot kernel + HTTP API server.
///
...
pub async fn run_daemon(
    ...
    info!("Skipper API server listening on http://{addr}");
