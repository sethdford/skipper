//! Channel bridge wiring — connects the Skipper kernel to channel adapters.
//!
//! Implements `ChannelBridgeHandle` on `SkipperKernel` and provides the
//! `start_channel_bridge()` entry point called by the daemon.

use skipper_channels::bridge::{BridgeManager, ChannelBridgeHandle};
use skipper_channels::discord::DiscordAdapter;
use skipper_channels::email::EmailAdapter;
use skipper_channels::google_chat::GoogleChatAdapter;
use skipper_channels::irc::IrcAdapter;
use skipper_channels::matrix::MatrixAdapter;
use skipper_channels::mattermost::MattermostAdapter;
use skipper_channels::rocketchat::RocketChatAdapter;
use skipper_channels::router::AgentRouter;
use skipper_channels::signal::SignalAdapter;
use skipper_channels::slack::SlackAdapter;
use skipper_channels::teams::TeamsAdapter;
use skipper_channels::telegram::TelegramAdapter;
use skipper_channels::twitch::TwitchAdapter;
use skipper_channels::types::ChannelAdapter;
use skipper_channels::whatsapp::WhatsAppAdapter;
use skipper_channels::xmpp::XmppAdapter;
use skipper_channels::zulip::ZulipAdapter;
// Wave 3
use skipper_channels::bluesky::BlueskyAdapter;
use skipper_channels::feishu::FeishuAdapter;
use skipper_channels::line::LineAdapter;
use skipper_channels::mastodon::MastodonAdapter;
use skipper_channels::messenger::MessengerAdapter;
use skipper_channels::reddit::RedditAdapter;
use skipper_channels::revolt::RevoltAdapter;
use skipper_channels::viber::ViberAdapter;
// Wave 4
use skipper_channels::flock::FlockAdapter;
use skipper_channels::guilded::GuildedAdapter;
use skipper_channels::keybase::KeybaseAdapter;
use skipper_channels::nextcloud::NextcloudAdapter;
use skipper_channels::nostr::NostrAdapter;
use skipper_channels::pumble::PumbleAdapter;
use skipper_channels::threema::ThreemaAdapter;
use skipper_channels::twist::TwistAdapter;
use skipper_channels::webex::WebexAdapter;
// Wave 5
use async_trait::async_trait;
use skipper_channels::dingtalk::DingTalkAdapter;
use skipper_channels::discourse::DiscourseAdapter;
use skipper_channels::gitter::GitterAdapter;
use skipper_channels::gotify::GotifyAdapter;
use skipper_channels::linkedin::LinkedInAdapter;
use skipper_channels::mumble::MumbleAdapter;
use skipper_channels::ntfy::NtfyAdapter;
use skipper_channels::webhook::WebhookAdapter;
use skipper_kernel::SkipperKernel;
use skipper_types::agent::AgentId;
use std::sync::Arc;
use std::time::{Duration, Instant};
use tracing::{error, info, warn};

/// Wraps `SkipperKernel` to implement `ChannelBridgeHandle`.
pub struct KernelBridgeAdapter {
    kernel: Arc<SkipperKernel>,
    started_at: Instant,
}
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn spawn_agent_by_name(&self, manifest_name: &str) -> Result<AgentId, String> {
        // Look for manifest at ~/.skipper/agents/{name}/agent.toml
    ...
    async fn uptime_info(&self) -> String {
        ...
        if hours > 0 {
            format!(
                "Skipper status: {}h {}m uptime, {} agent(s)",
                hours,
                mins,
                agents.len()
            )
            ...
            format!(
                "Skipper status: {}m uptime, {} agent(s)",
                mins,
                agents.len()
            )
        }
    ...
    async fn list_providers_text(&self) -> String {
        ...
        for p in catalog.list_providers() {
            let status = match p.auth_status {
                skipper_types::model_catalog::AuthStatus::Configured => "configured",
                skipper_types::model_catalog::AuthStatus::Missing => "not configured",
                skipper_types::model_catalog::AuthStatus::NotRequired => "local (no key needed)",
            };
            msg.push_str(&format!(
                "  {} — {} [{}, {} model(s)]\n",
    ...
    async fn list_skills_text(&self) -> String {
        ...
        if skills.is_empty() {
            return "No skills installed. Place skills in ~/.skipper/skills/ or install from the marketplace.".to_string();
        }
        ...
        let result = self
            ...
            .execute_run(
                ...
                |step_agent| match step_agent {
                    skipper_kernel::workflow::StepAgent::ById { id } => {
                        let aid: AgentId = id.parse().ok()?;
                        let entry = registry_ref.get(aid)?;
                        Some((aid, entry.name.clone()))
                    }
                    skipper_kernel::workflow::StepAgent::ByName { name } => {
    ...
    async fn list_schedules_text(&self) -> String {
        ...
        for job in &jobs {
            ...
            let sched = match &job.schedule {
                skipper_types::scheduler::CronSchedule::Cron { expr, .. } => expr.clone(),
                skipper_types::scheduler::CronSchedule::Every { every_secs } => {
                    format!("every {every_secs}s")
                }
                skipper_types::scheduler::CronSchedule::At { at } => {
    ...
    async fn manage_schedule_text(&self, action: &str, args: &[String]) -> String {
        match action {
            "add" => {
                ...
                let job = skipper_types::scheduler::CronJob {
                    id: skipper_types::scheduler::CronJobId::new(),
                    agent_id: agent.id,
                    name: format!("chat-{}", &agent.name),
                    enabled: true,
                    schedule: skipper_types::scheduler::CronSchedule::Cron {
                        expr: cron_expr.clone(),
                        tz: None,
                    },
                    action: skipper_types::scheduler::CronAction::AgentTurn {
                        message: message.clone(),
                        model_override: None,
                        timeout_secs: None,
                    },
                    delivery: skipper_types::scheduler::CronDelivery::None,
    ...
    async fn channel_overrides(
        &self,
        channel_type: &str,
    ) -> Option<skipper_types::config::ChannelOverrides> {
    ...
    async fn authorize_channel_user(
        ...
        let auth_action = match action {
            "chat" => skipper_kernel::auth::Action::ChatWithAgent,
            "spawn" => skipper_kernel::auth::Action::SpawnAgent,
            "kill" => skipper_kernel::auth::Action::KillAgent,
            "install_skill" => skipper_kernel::auth::Action::InstallSkill,
            _ => skipper_kernel::auth::Action::ChatWithAgent,
        };
    ...
    async fn record_delivery(
        ...
        let receipt = if success {
            skipper_kernel::DeliveryTracker::sent_receipt(channel, recipient)
        } else {
            skipper_kernel::DeliveryTracker::failed_receipt(
                channel,
                recipient,
    ...
    async fn a2a_agents_text(&self) -> String {
        ...
        for (url, card) in agents.iter() {
            ...
            if !desc.is_empty() {
                let short = skipper_types::truncate_str(desc, 60);
...
fn parse_trigger_pattern(s: &str) -> Option<skipper_kernel::triggers::TriggerPattern> {
    use skipper_kernel::triggers::TriggerPattern;
...
pub async fn start_channel_bridge(kernel: Arc<SkipperKernel>) -> Option<BridgeManager> {
...
pub async fn start_channel_bridge_with_config(
    kernel: Arc<SkipperKernel>,
    config: &skipper_types::config::ChannelsConfig,
...
pub async fn reload_channels_from_disk(
    ...
    let fresh_config = skipper_kernel::config::load_config(Some(&config_path));
