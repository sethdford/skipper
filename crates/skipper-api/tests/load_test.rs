//! Load & performance tests for the Skipper API.
//!
//! Measures throughput under concurrent access: agent spawning, API endpoint
//! latency, session management, and memory usage.
//!
//! Run: cargo test -p skipper-api --test load_test -- --nocapture

use axum::Router;
use skipper_api::middleware;
use skipper_api::routes::{self, AppState};
use skipper_kernel::SkipperKernel;
use skipper_types::config::{DefaultModelConfig, KernelConfig};
use std::sync::Arc;
...
async fn start_test_server() -> TestServer {
    ...
    let kernel = SkipperKernel::boot_with_config(config).expect("Kernel should boot");
...
async fn load_metrics_sustained() {
    ...
    for _ in 0..n {
        ...
        assert!(body.contains("skipper_agents_active"));
    }
