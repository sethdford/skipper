...
//! context files (AGENTS.md, SOUL.md, TOOLS.md, IDENTITY.md, HEARTBEAT.md), and Skipper
...
pub struct WorkspaceContext {
    ...
    /// Whether .skipper/ directory exists.
    pub has_skipper_dir: bool,
...
impl WorkspaceContext {
    ...
    pub fn detect(root: &Path) -> Self {
        ...
        let has_skipper_dir = root.join(".skipper").exists();
...
/// Persistent workspace state, saved to `.skipper/workspace-state.json`.
...
impl WorkspaceState {
    /// Load state from the workspace's `.skipper/workspace-state.json`.
    ...
    /// Save state to the workspace's `.skipper/workspace-state.json`.
    pub fn save(&self, workspace_root: &Path) -> Result<(), String> {
        let dir = workspace_root.join(".skipper");
        std::fs::create_dir_all(&dir)
            .map_err(|e| format!("Failed to create .skipper dir: {e}"))?;
...
mod tests {
    ...
    fn test_detect_rust_project() {
        let dir = std::env::temp_dir().join("skipper_ws_rust_test");
    ...
    fn test_detect_node_project() {
        let dir = std::env::temp_dir().join("skipper_ws_node_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_detect_python_project() {
        let dir = std::env::temp_dir().join("skipper_ws_py_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_detect_go_project() {
        let dir = std::env::temp_dir().join("skipper_ws_go_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_detect_unknown_project() {
        let dir = std::env::temp_dir().join("skipper_ws_unk_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_workspace_context_detect() {
        let dir = std::env::temp_dir().join("skipper_ws_ctx_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_get_file_cache_hit() {
        let dir = std::env::temp_dir().join("skipper_ws_cache_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_file_size_cap() {
        let dir = std::env::temp_dir().join("skipper_ws_cap_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_build_context_section() {
        let dir = std::env::temp_dir().join("skipper_ws_section_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join("Cargo.toml"), "[package]").unwrap();
        std::fs::create_dir_all(dir.join(".git")).unwrap();
    ...
    fn test_workspace_state_round_trip() {
        let dir = std::env::temp_dir().join("skipper_ws_state_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_workspace_state_missing_file() {
        let dir = std::env::temp_dir().join("skipper_ws_state_missing");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
