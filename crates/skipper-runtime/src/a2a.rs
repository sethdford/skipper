...
//! - `build_agent_card` — expose Skipper agents via A2A
...
pub struct AgentCard {
    ...
    /// Skills this agent can perform (A2A skill descriptors, not Skipper skills).
...
/// A2A skill descriptor (not an Skipper skill — describes a capability).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub async fn discover_external_agents(
    agents: &[skipper_types::config::ExternalAgent],
...
// ---------------------------------------------------------------------------
// A2A Server — expose Skipper agents via A2A
// ---------------------------------------------------------------------------

/// Build an A2A Agent Card from an Skipper agent manifest.
...
impl A2aClient {
    ...
    pub async fn discover(&self, url: &str) -> Result<AgentCard, String> {
        ...
        let response = self
            .client
            .get(&agent_json_url)
            .header("User-Agent", "Skipper/0.1 A2A")
            .send()
            .await
...
mod tests {
    ...
    fn test_a2a_config_serde() {
        use skipper_types::config::{A2aConfig, ExternalAgent};
