...
use skipper_types::config::DockerSandboxConfig;
use std::path::Path;
...
mod tests {
    ...
    fn test_sanitize_container_name_valid() {
        let result = sanitize_container_name("skipper-sandbox-abc123").unwrap();
        assert_eq!(result, "skipper-sandbox-abc123");
    }
    ...
    fn test_config_defaults() {
        ...
        assert_eq!(config.container_prefix, "skipper-sandbox");
