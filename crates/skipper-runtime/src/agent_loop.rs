...
use skipper_memory::session::Session;
...
use skipper_types::memory::{Memory, MemoryFilter, MemorySource};
use skipper_types::message::{
    ContentBlock, Message, MessageContent, Role, StopReason, TokenUsage,
};
use skipper_types::tool::{ToolCall, ToolDefinition};
...
pub struct AgentLoopResult {
    ...
    pub directives: skipper_types::message::ReplyDirectives,
}

/// Run the agent execution loop for a single user message.
///
/// This is the core of Skipper: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: skipper_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if text.trim() == "NO_REPLY" || parsed_directives.silent {
                    ...
                    return Ok(AgentLoopResult {
                        ...
                        cost_usd: None,
                        silent: true,
                        directives: skipper_types::message::ReplyDirectives {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: skipper_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in &response.tool_calls {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(SkipperError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: skipper_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: skipper_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    Err(SkipperError::MaxIterationsExceeded(max_iterations))
}
...
) -> SkipperResult<crate::llm_driver::CompletionResponse> {
    ...
    if let (Some(provider), Some(cooldown)) = (provider, cooldown) {
        match cooldown.check(provider) {
            CooldownVerdict::Reject {
                ...
                return Err(SkipperError::LlmDriver(format!(
    ...
    for attempt in 0..=MAX_RETRIES {
        match driver.complete(request.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(SkipperError::LlmDriver(user_msg));
            }
        }
    }

    Err(SkipperError::LlmDriver(
...
mod tests {
    ...
    use skipper_types::tool::ToolCall;
    ...
    fn test_manifest() -> AgentManifest {
        AgentManifest {
            ...
            model: skipper_types::agent::ModelConfig {
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = skipper_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = skipper_types::agent::AgentId::new();
        let mut session = skipper_memory::session::Session {
            id: skipper_types::agent::SessionId::new(),
            agent_id,
            messages: Vec::new(),
