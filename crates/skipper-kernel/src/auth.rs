...
//! to Skipper users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use skipper_types::agent::UserId;
use skipper_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their Skipper user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the Skipper UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> SkipperResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| SkipperError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(SkipperError::AuthDenied(format!(
