//! Cron job scheduler engine for the Skipper kernel.
//!
...
use skipper_types::scheduler::{CronJob, CronJobId, CronSchedule};
...
///
/// The `CronJob` struct in `skipper-types` is intentionally lean (no
...
impl CronScheduler {
    ...
    ///
    /// `home_dir` is the Skipper data directory; jobs are persisted to
    ...
    pub fn load(&self) -> SkipperResult<usize> {
        if !self.persist_path.exists() {
            return Ok(0);
        }
        let data = std::fs::read_to_string(&self.persist_path)
            .map_err(|e| SkipperError::Internal(format!("Failed to read cron jobs: {e}")))?;
        let metas: Vec<JobMeta> = serde_json::from_str(&data)
            .map_err(|e| SkipperError::Internal(format!("Failed to parse cron jobs: {e}")))?;
        let count = metas.len();
        for meta in metas {
    ...
    pub fn persist(&self) -> SkipperResult<()> {
        let metas: Vec<JobMeta> = self.jobs.iter().map(|r| r.value().clone()).collect();
        let data = serde_json::to_string_pretty(&metas)
            .map_err(|e| SkipperError::Internal(format!("Failed to serialize cron jobs: {e}")))?;
        let tmp_path = self.persist_path.with_extension("json.tmp");
        std::fs::write(&tmp_path, data.as_bytes()).map_err(|e| {
            SkipperError::Internal(format!("Failed to write cron jobs temp file: {e}"))
        })?;
        std::fs::rename(&tmp_path, &self.persist_path).map_err(|e| {
            SkipperError::Internal(format!("Failed to rename cron jobs file: {e}"))
        })?;
    ...
    pub fn add_job(&self, mut job: CronJob, one_shot: bool) -> SkipperResult<CronJobId> {
        // Global limit
        let max_jobs = self.max_total_jobs.load(Ordering::Relaxed);
        if self.jobs.len() >= max_jobs {
            return Err(SkipperError::Internal(format!(
        ...
        job.validate(agent_count)
            .map_err(SkipperError::InvalidInput)?;
    ...
    pub fn remove_job(&self, id: CronJobId) -> SkipperResult<CronJob> {
        self.jobs
            .remove(&id)
            .map(|(_, meta)| meta.job)
            .ok_or_else(|| SkipperError::Internal(format!("Cron job {id} not found")))
    }

    /// Enable or disable a job. Re-enabling resets errors and recomputes
    /// `next_run`.
    pub fn set_enabled(&self, id: CronJobId, enabled: bool) -> SkipperResult<()> {
        match self.jobs.get_mut(&id) {
            ...
            None => Err(SkipperError::Internal(format!("Cron job {id} not found"))),
        }
    }
    ...
    pub fn record_failure(&self, id: CronJobId, error_msg: &str) {
        if let Some(mut meta) = self.jobs.get_mut(&id) {
            ...
            meta.last_status = Some(format!(
                "error: {}",
                skipper_types::truncate_str(error_msg, 256)
            ));
...
mod tests {
    use super::*;
    use chrono::Duration;
    use skipper_types::scheduler::{CronAction, CronDelivery};
    ...
    fn test_add_job_per_agent_limit() {
        // MAX_JOBS_PER_AGENT = 50 in skipper-types
