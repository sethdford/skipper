...
use dashmap::DashMap;
use skipper_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> SkipperResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(SkipperError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> SkipperResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: skipper_types::agent::SessionId,
    ) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: skipper_types::agent::AgentIdentity,
    ) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_skills(&self, id: AgentId, skills: Vec<String>) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_mcp_servers(&self, id: AgentId, servers: Vec<String>) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> SkipperResult<()> {
        if self.name_index.contains_key(&new_name) {
            return Err(SkipperError::AgentAlreadyExists(new_name));
        }
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> SkipperResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SkipperError::AgentNotFound(id.to_string()))?;
