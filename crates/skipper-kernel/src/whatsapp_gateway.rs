...
//!
//! Embeds the gateway JS at compile time, extracts it to `~/.skipper/whatsapp-gateway/`,
//! runs `npm install` if needed, and spawns `node index.js` as a managed child process
//! that auto-restarts on crash.

use crate::config::skipper_home;
...
fn gateway_dir() -> PathBuf {
    skipper_home().join("whatsapp-gateway")
}
...
pub async fn start_whatsapp_gateway(kernel: &Arc<super::kernel::SkipperKernel>) {
    ...
    let skipper_url = format!("http://{api_listen}");
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let child = tokio::process::Command::new(node_cmd)
                ...
                .env("SKIPPER_URL", &skipper_url)
                .env("SKIPPER_DEFAULT_AGENT", &default_agent)
...
mod tests {
    ...
    fn test_embedded_files_not_empty() {
        ...
        assert!(GATEWAY_PACKAGE_JSON.contains("@skipper/whatsapp-gateway"));
    }
    ...
    #[test]
    fn test_gateway_dir_under_skipper_home() {
        let dir = gateway_dir();
    ...
    fn test_write_if_changed_creates_new_file() {
        let tmp = std::env::temp_dir().join("skipper_test_gateway");
