...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p skipper-kernel --test multi_agent_test -- --nocapture
...
fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("skipper-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = SkipperKernel::boot_with_config(test_config()).expect("Kernel should boot");
...
"#,
    ...
    println!("\n{}", "=".repeat(60));
    println!("  SKIPPER MULTI-AGENT FLEET TEST");
