...
impl UsageStore {
    ...
    pub fn record(&self, record: &UsageRecord) -> SkipperResult<()> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn query_hourly(&self, agent_id: AgentId) -> SkipperResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_daily(&self, agent_id: AgentId) -> SkipperResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_monthly(&self, agent_id: AgentId) -> SkipperResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_global_hourly(&self) -> SkipperResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_global_monthly(&self) -> SkipperResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_summary(&self, agent_id: Option<AgentId>) -> SkipperResult<UsageSummary> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_by_model(&self) -> SkipperResult<Vec<ModelUsage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
        ...
        for row in rows {
            results.push(row.map_err(|e| SkipperError::Memory(e.to_string()))?);
        }
        Ok(results)
    }

    /// Query daily usage breakdown for the last N days.
    pub fn query_daily_breakdown(&self, days: u32) -> SkipperResult<Vec<DailyBreakdown>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_first_event_date(&self) -> SkipperResult<Option<String>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_today_cost(&self) -> SkipperResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn cleanup_old(&self, days: u32) -> SkipperResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
