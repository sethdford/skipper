...
use chrono::Utc;
use skipper_types::agent::{AgentId, SessionId};
use skipper_types::error::{SkipperError, SkipperResult};
use skipper_types::message::{ContentBlock, Message, MessageContent, Role};
...
impl SessionStore {
        ...
        match result {
            Ok((agent_str, messages_blob, tokens, label)) => {
                ...
                let messages: Vec<Message> = rmp_serde::from_slice(&messages_blob)
                    .map_err(|e| SkipperError::Serialization(e.to_string()))?;
                Ok(Some(Session {
                    id: session_id,
                    agent_id,
                    messages,
    ...
    pub fn delete_agent_sessions(&self, agent_id: AgentId) -> SkipperResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn list_sessions(&self) -> SkipperResult<Vec<serde_json::Value>> {
        ...
        for row in rows {
            sessions.push(row.map_err(|e| SkipperError::Memory(e.to_string()))?);
        }
        Ok(sessions)
    }
...
impl SessionStore {
    ...
    pub fn load_canonical(&self, agent_id: AgentId) -> SkipperResult<CanonicalSession> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    ) -> SkipperResult<CanonicalSession> {
        let mut canonical = self.load_canonical(agent_id)?;
            ...
            if to_compact > canonical.compaction_cursor {
                ...
                for msg in compacting {
                    let role = match msg.role {
                        skipper_types::message::Role::User => "User",
                        skipper_types::message::Role::Assistant => "Assistant",
                        skipper_types::message::Role::System => "System",
                    };
                    let text = msg.content.text_content();
                    if !text.is_empty() {
                        // Truncate individual messages in summary to keep it compact (UTF-8 safe)
                        let truncated = if text.len() > 200 {
                            format!("{}...", skipper_types::truncate_str(&text, 200))
                        } else {
                            text
                        };
    ...
    ) -> SkipperResult<(Option<String>, Vec<Message>)> {
    ...
    fn save_canonical(&self, canonical: &CanonicalSession) -> SkipperResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
...
mod tests {
    ...
    fn test_jsonl_mirror_write() {
        ...
        session
            .messages
            .push(skipper_types::message::Message::user("Hello"));
        session
            .messages
            .push(skipper_types::message::Message::assistant("Hi there!"));
        store.save_session(&session).unwrap();
