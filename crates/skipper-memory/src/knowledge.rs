...
impl KnowledgeStore {
    ...
    pub fn add_entity(&self, entity: Entity) -> SkipperResult<String> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn add_relation(&self, relation: Relation) -> SkipperResult<String> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn query_graph(&self, pattern: GraphPattern) -> SkipperResult<Vec<GraphMatch>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
        ...
        for row_result in rows {
            let r = row_result.map_err(|e| SkipperError::Memory(e.to_string()))?;
