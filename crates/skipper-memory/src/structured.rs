...
use chrono::Utc;
use skipper_types::agent::{AgentEntry, AgentId};
...
impl StructuredStore {
    ...
    pub fn get(&self, agent_id: AgentId, key: &str) -> SkipperResult<Option<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn set(
        ...
        let conn = self
            ...
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
        let blob =
            serde_json::to_vec(&value).map_err(|e| SkipperError::Serialization(e.to_string()))?;
    ...
    pub fn delete(&self, agent_id: AgentId, key: &str) -> SkipperResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SkipperError::Internal(e.to_string()))?;
    ...
    pub fn list_kv(&self, agent_id: AgentId) -> SkipperResult<Vec<(String, serde_json::Value)>> {
        ...
        for row in rows {
            let (key, val_str) = row.map_err(|e| SkipperError::Memory(e.to_string()))?;
        ...
        match result {
            Ok((name, manifest_blob, state_str, created_str, session_id_str)) => {
                ...
                let session_id = session_id_str
                    ...
                    .map(skipper_types::agent::SessionId)
                    .unwrap_or_else(skipper_types::agent::SessionId::new);
                Ok(Some(AgentEntry {
                    id: agent_id,
                    name,
                    manifest,
    ...
    pub fn load_all_agents(&self) -> SkipperResult<Vec<AgentEntry>> {
        ...
        for row in rows {
            ...
            let agent_id = match uuid::Uuid::parse_str(&id_str).map(skipper_types::agent::AgentId)
            {
                Ok(id) => id,
                Err(e) => {
            ...
            let manifest: skipper_types::agent::AgentManifest = match rmp_serde::from_slice(
                &manifest_blob,
            ) {
                Ok(m) => m,
                Err(e) => {
    ...
    pub fn list_agents(&self) -> SkipperResult<Vec<(String, String, String)>> {
        ...
        let mut agents = Vec::new();
        for row in rows {
            agents.push(row.map_err(|e| SkipperError::Memory(e.to_string()))?);
        }
        Ok(agents)
    }
}
