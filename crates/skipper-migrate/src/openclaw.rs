...
// ---------------------------------------------------------------------------
// Skipper output types (TOML)
// ---------------------------------------------------------------------------

/// Skipper config.toml structure for serialization.
#[derive(Serialize)]
struct SkipperConfig {
    default_model: SkipperModelConfig,
    memory: SkipperMemorySection,
    network: SkipperNetworkSection,
    #[serde(skip_serializing_if = "Option::is_none")]
    channels: Option<toml::Value>,
}

#[derive(Serialize)]
struct SkipperModelConfig {
    provider: String,
    model: String,
    api_key_env: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Serialize)]
struct SkipperMemorySection {
    decay_rate: f32,
}

#[derive(Serialize)]
struct SkipperNetworkSection {
    listen_addr: String,
}
...
/// Map OpenClaw DM policy to Skipper DM policy string.
...
/// Map OpenClaw group policy to Skipper group policy string.
...
use skipper_types::tool_compat::{is_known_skipper_tool, map_tool_name};

/// Map OpenClaw tool profile to Skipper capability tool list.
...
/// Map OpenClaw provider name to Skipper provider name.
...
fn migrate_config_from_json(
    ...
    let of_config = SkipperConfig {
        ...
        memory: SkipperMemorySection { decay_rate: 0.05 },
        network: SkipperNetworkSection {
    ...
    let config_content = format!(
        "# Skipper Agent OS configuration\n\
...
fn migrate_channels_from_json(
    ...
    // --- BlueBubbles (skip — no Skipper adapter) ---
    if oc_channels.bluebubbles.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: "bluebubbles".to_string(),
            reason: "No Skipper adapter available — consider using the iMessage channel instead"
                .to_string(),
        });
    }
    ...
    for key in oc_channels.other.keys() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: key.clone(),
            reason: format!("Unknown channel '{key}' — not mapped to any Skipper adapter"),
        });
    }
            ...
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{id}': tool '{tool}' has no Skipper equivalent and was skipped"
                    ));
                }
...
fn convert_agent_from_json(
    ...
    let system_prompt = entry
        ...
        .unwrap_or_else(|| {
            format!(
                "You are {display_name}, an AI agent running on the Skipper Agent OS. You are helpful, concise, and accurate."
            )
        });
    ...
    toml_str.push_str("author = \"skipper\"\n");
...
fn report_skipped_features(root: &OpenClawRoot, source: &Path, report: &mut MigrationReport) {
    ...
    if root.cron.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Config,
            name: "cron".to_string(),
            reason: "Cron job scheduling not yet supported — use Skipper's ScheduleMode::Periodic instead".to_string(),
        });
    }
    ...
    if root.hooks.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Webhook hooks not supported — use Skipper's event system instead".to_string(),
        });
    }
        ...
        if let Some(ref entries) = skills.entries {
            if !entries.is_empty() {
                report.skipped.push(SkippedItem {
                    ...
                    reason: "Skills must be reinstalled via `skipper skill install`".to_string(),
                });
            }
        }
    }
    ...
    if source.join("memory-search").join("index.db").exists() {
        report.skipped.push(SkippedItem {
            ...
            reason: "SQLite vector index not portable — Skipper will rebuild embeddings"
                .to_string(),
        });
    }
    ...
    if root.session.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Session scope config differs — Skipper uses per-agent sessions by default"
                .to_string(),
        });
    }
    ...
    if root.memory.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason:
                "Memory backend config not migrated — Skipper uses SQLite with vector embeddings"
                    .to_string(),
        });
    }
}
        ...
        match *name {
            ...
            "bluebubbles" => {
                report.skipped.push(SkippedItem {
                    kind: ItemKind::Channel,
                    name: "bluebubbles".to_string(),
                    reason: "No Skipper adapter available — consider using the iMessage channel instead".to_string(),
                });
            }
            _ => {}
        }
    ...
    for entry in entries {
        ...
        match convert_legacy_agent(&agent_yaml, &agent_name) {
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{agent_name}': tool '{tool}' has no Skipper equivalent and was skipped"
                    ));
                }
...
fn convert_legacy_agent(
    ...
    let system_prompt = oc.system_prompt.unwrap_or_else(|| {
        format!(
            "You are {}, an AI agent running on the Skipper Agent OS. {}",
            oc.name,
            ...
            for entry in entries.flatten() {
                ...
                if has_package_json && has_index {
                    report.skipped.push(SkippedItem {
                        kind: ItemKind::Skill,
                        name: name.clone(),
                        reason: "Node.js skill — run with `skipper skill install` after migration"
                            .to_string(),
                    });
                } else {
                    report.skipped.push(SkippedItem {
...
mod tests {
  ...
  agents: {
    ...
    #[test]
    fn test_is_known_skipper_tool() {
        assert!(is_known_skipper_tool("file_read"));
        assert!(is_known_skipper_tool("shell_exec"));
        assert!(is_known_skipper_tool("web_fetch"));
        assert!(!is_known_skipper_tool("Read"));
        assert!(!is_known_skipper_tool("unknown"));
    }
