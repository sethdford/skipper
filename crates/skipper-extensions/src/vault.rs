...
//!
//! Stores secrets in `~/.skipper/vault.enc`, with the master key sourced from
//! the OS keyring (Windows Credential Manager / macOS Keychain / Linux Secret Service)
//! or the `SKIPPER_VAULT_KEY` env var for headless/CI environments.
...
impl CredentialVault {
    ...
    pub fn unlock(&mut self) -> ExtensionResult<()> {
        ...
        if !self.path.exists() {
            return Err(ExtensionError::Vault(
                "Vault not initialized. Run `skipper vault init`.".to_string(),
            ));
        }
...
fn machine_fingerprint() -> Vec<u8> {
    ...
    hasher.update(b"skipper-vault-v1");
