...
pub fn default_client_ids() -> HashMap<&'static str, &'static str> {
    ...
    m.insert("google", "skipper-google-client-id");
    m.insert("github", "skipper-github-client-id");
    m.insert("microsoft", "skipper-microsoft-client-id");
    m.insert("slack", "skipper-slack-client-id");
    m
}

/// Resolve OAuth client IDs with config overrides applied on top of defaults.
pub fn resolve_client_ids(
    config: &skipper_types::config::OAuthConfig,
...
mod tests {
    ...
    fn resolve_client_ids_uses_defaults() {
        let config = skipper_types::config::OAuthConfig::default();
        let ids = resolve_client_ids(&config);
        assert_eq!(ids["google"], "skipper-google-client-id");
        assert_eq!(ids["github"], "skipper-github-client-id");
    }

    #[test]
    fn resolve_client_ids_applies_overrides() {
        let config = skipper_types::config::OAuthConfig {
        ...
        assert_eq!(ids["github"], "skipper-github-client-id"); // default
        assert_eq!(ids["microsoft"], "my-msft-id");
        assert_eq!(ids["slack"], "skipper-slack-client-id"); // default
    }
}
