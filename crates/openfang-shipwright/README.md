...
- **10+ Signal Collectors**: Security (CVE parsing), Dependency updates, Coverage regression, Dead code, Test flakiness, Architecture violations, and Hand signals from other OpenFang Hands
...
```bash
openfang shipwright pipeline start --issue 123
openfang shipwright fleet status
openfang shipwright decide run
openfang shipwright memory search "undefined property"
openfang shipwright dora --repo myorg/myrepo
...
Shipwright integrates with OpenFang's kernel via:

- **KernelHandle**: For scheduling, RBAC, metering, agent spawning
- **Memory Store**: openfang-memory for vector embeddings
...
```bash
cargo test -p openfang-shipwright
```
...
```bash
# Start daemon
GITHUB_TOKEN=ghp_... cargo run --release --bin openfang start &
sleep 6
...
```rust
use openfang_shipwright::Pipeline;
...
```rust
use openfang_shipwright::ShipwrightMemory;
...
```rust
use openfang_shipwright::IntelligenceEngine;
...
- **Memory Search**: O(1) with DashMap + semantic similarity in openfang-memory
...
1. Store failure patterns first:
   ```bash
   openfang shipwright memory import-jsonl path/to/failures.jsonl
   ```
...
Same as OpenFang (see root LICENSE)
