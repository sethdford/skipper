...
const SKIPPER_URL = (process.env.SKIPPER_URL || 'http://127.0.0.1:4200').replace(/\/+$/, '');
const DEFAULT_AGENT = process.env.SKIPPER_DEFAULT_AGENT || 'assistant';
...
async function startConnection() {
  ...
  // Incoming messages → forward to Skipper
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Forward to Skipper agent
      try {
        const response = await forwardToSkipper(text, phone, pushName);
        if (response && sock) {
...
// ---------------------------------------------------------------------------
// Forward incoming message to Skipper API, return agent response
// ---------------------------------------------------------------------------
function forwardToSkipper(text, phone, pushName) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(`${SKIPPER_URL}/api/agents/${encodeURIComponent(DEFAULT_AGENT)}/message`);
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('Skipper API timeout'));
    });
    req.write(payload);
    req.end();
  });
...
// ---------------------------------------------------------------------------
// Send a message via Baileys (called by Skipper for outgoing)
// ---------------------------------------------------------------------------
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[gateway] Skipper URL: ${SKIPPER_URL}`);
