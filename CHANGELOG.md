...
#### Client SDKs
- JavaScript SDK (`@skipper/sdk`): full REST API client with streaming, TypeScript declarations
- Python client SDK (`skipper_client`): zero-dependency stdlib client with SSE streaming
- Python agent SDK (`skipper_sdk`): decorator-based framework for writing Python agents
...
- MCP server (exposes Skipper tools via MCP protocol)
