"""
Skipper Python SDK — helper library for writing Python agents.
...
Or for simple scripts without the decorator pattern:
    ...
    from skipper_sdk import read_input, respond
...
def read_input() -> Dict[str, Any]:
    """Read the input JSON from stdin (sent by the Skipper kernel)."""
    line = sys.stdin.readline().strip()
    if not line:
        # Fallback: check environment variables
        agent_id = os.environ.get("SKIPPER_AGENT_ID", "")
        message = os.environ.get("SKIPPER_MESSAGE", "")
        return {
            "type": "message",
            "agent_id": agent_id,
            "message": message,
...
def respond(text: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    """Send a response back to the Skipper kernel via stdout."""
...
def log(message: str, level: str = "info") -> None:
    """Log a message to stderr (visible in Skipper daemon logs)."""
...
if __name__ == "__main__":
    print("Skipper Python SDK")
    print("====================")
    print()
    print("Import this module in your agent scripts:")
    print()
    print("  from skipper_sdk import Agent")
    print()
    print("  agent = Agent()")
    print()
