...
1. **Add token estimation & context guard** (`crates/skipper-runtime/src/compactor.rs`)
...
2. **Add in-loop token guard** (`crates/skipper-runtime/src/agent_loop.rs`)
...
3. **Filter tools by profile in kernel** (`crates/skipper-kernel/src/kernel.rs`)
...
4. **Raise default token quota** (`crates/skipper-types/src/agent.rs`)
   - Change `max_llm_tokens_per_hour` from 100_000 to 1_000_000
   - 100K is too low — a single system prompt is 30-40K tokens

5. **Token-based compaction trigger** (`crates/skipper-kernel/src/kernel.rs`)
...
6. **Compact system prompt injections** (`crates/skipper-kernel/src/kernel.rs`)
...
**Problem:** Desktop app may show Tauri default icons. Branding assets exist at `~/Downloads/skipper/output/` but aren't installed.

**What to do:**

1. Generate all required icon sizes from source PNG (`skipper-logo-transparent.png`, 2000x2000)
2. Place into `crates/skipper-desktop/icons/`:
   - `icon.png` (1024x1024)
   - `icon.ico` (multi-size: 256, 128, 64, 48, 32, 16)
   - `32x32.png`
   - `128x128.png`
   - `128x128@2x.png` (256x256)
3. Replace web UI logo at `crates/skipper-api/static/logo.png`
4. Update favicon if one exists

**Assets available:**
- `skipper-logo-transparent.png` (328KB, 2000x2000) — primary source
- `skipper-logo-black-bg.png` (312KB) — for dark contexts
- `skipper-vector-transparent.svg` (293KB) — scalable vector
- `skipper-animated.svg` (310KB) — for loading screens

**Done when:**
- Desktop app shows Skipper logo in taskbar, title bar, and installer
...
**Status: COMPLETE** — Generated Ed25519 signing keypair via `cargo tauri signer generate --ci`. Public key installed in `tauri.conf.json`. Private key at `~/.tauri/skipper.key`. Set `TAURI_SIGNING_PRIVATE_KEY_PATH` in CI secrets.
...
These close the gaps that would make users pick OpenClaw over Skipper.
...
**Files:** `crates/skipper-api/static/js/pages/chat.js`, `crates/skipper-runtime/src/tool_runner.rs`
...
**Problem:** `skipper.sh` domain isn't set up. Users can't do `curl -sSf https://skipper.sh | sh`.

**What to do:**
1. Set up GitHub Pages or Cloudflare Worker for skipper.sh
...
**Done when:**
- `curl -sSf https://skipper.sh | sh` installs the latest release
- `irm https://skipper.sh/install.ps1 | iex` works on Windows PowerShell
...
These are features where Skipper can leapfrog OpenClaw.
...
**Status: COMPLETE** — Created `sdk/javascript/` (@skipper/sdk) with full REST client: agent CRUD, streaming via SSE, sessions, workflows, skills, channels, memory KV, triggers, schedules + TypeScript declarations. Created `sdk/python/skipper_client.py` (zero-dependency stdlib urllib) with same coverage. Both include basic + streaming examples. Python `setup.py` for pip install.
...
1. Create `sdks/javascript/` — thin wrapper around REST API
   ...
   - Publish to npm as `@skipper/sdk`
2. Create `sdks/python/` — thin wrapper with httpx
   - Same operations
   - Publish to PyPI as `skipper`
3. Include usage examples in README

**Done when:**
- `npm install @skipper/sdk` works
- `pip install skipper` works
...
**Status: COMPLETE** — Added `GET /api/metrics` endpoint returning Prometheus text format. Metrics: `skipper_uptime_seconds`, `skipper_agents_active`, `skipper_agents_total`, `skipper_tokens_total{agent,provider,model}`, `skipper_tool_calls_total{agent}`, `skipper_panics_total`, `skipper_restarts_total`, `skipper_info{version}`.

**Problem (was):** No way to monitor Skipper in production (no Prometheus, no OpenTelemetry).

**What to do:**
1. Add `/api/metrics` endpoint with Prometheus format
   - `skipper_agents_active` gauge
   - `skipper_messages_total` counter (by agent, by channel)
   - `skipper_tokens_total` counter (by provider, by model)
   - `skipper_request_duration_seconds` histogram
   - `skipper_tool_calls_total` counter (by tool name)
   - `skipper_errors_total` counter (by type)
2. Optional: OTLP export for tracing spans

**Files:** `crates/skipper-api/src/routes.rs`, new `metrics.rs` module
...
**Problem (was):** Both Skipper and OpenClaw define workflows in TOML/config only. No visual builder exists in either. First to ship this wins.
...
**Files:** `crates/skipper-kernel/src/kernel.rs`, `routes.rs`, `ws.rs`, `index_body.html`
...
**What to do:**
1. Watch `~/.skipper/config.toml` for changes (notify crate)
2. On change: re-parse, diff, apply only changed sections
3. Log what was reloaded
4. UI notification: "Config reloaded"

**Files:** `crates/skipper-api/src/server.rs`, `crates/skipper-types/src/config.rs`
...
**Status: ALL CODE COMPLETE** — All 18 code items done. 1751 tests passing. Production audit completed: 2 critical bugs fixed (API delete alias, config/set route), CSP hardened (Tauri + middleware), Tauri signing key installed. Remaining for release: tag v0.1.0, build release artifacts, set up skipper.sh domain.
...
| Feature | OpenClaw | Skipper | Winner |
|---------|----------|----------|--------|
| Language/Performance | Node.js (~200MB) | Rust (~30MB single binary) | **Skipper** |
| Channels | ~15 | **40** | **Skipper** |
| Built-in Tools | ~19 | **41** | **Skipper** |
| Security Systems | Token + sandbox | **16 defense systems** | **Skipper** |
| Agent Templates | Manual config | **30 pre-configured** | **Skipper** |
| Hands (autonomous) | None | **7 packages** | **Skipper** |
| Workflow Engine | Cron + webhooks | **Full DAG with parallel/loops** | **Skipper** |
| Knowledge Graph | Flat vector store | **Entity-relation graph** | **Skipper** |
| P2P Networking | None | **OFP wire protocol** | **Skipper** |
| WASM Sandbox | Docker only | **Dual-metered WASM** | **Skipper** |
| Desktop App | Electron (~200MB) | **Tauri (~30MB)** | **Skipper** |
| Migration | N/A | **`migrate --from openclaw`** | **Skipper** |
| Skills | 54 bundled | **60 bundled** | **Skipper** |
| LLM Providers | ~15 | **27 providers, 130+ models** | **Skipper** |
...
| Visual Workflow Builder | None | **Drag-and-drop builder** | **Skipper** |

**Skipper wins 15/18 categories.** The remaining gaps are: mobile apps (OpenClaw), voice wake word (OpenClaw slight edge).
...
Sprint 2: 4/5 COMPLETE
  ...
  2.4 Install script domain ........ PENDING (infra: set up skipper.sh domain)
