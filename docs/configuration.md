# Skipper Configuration Reference

Complete reference for `config.toml`, covering every configurable field in the Skipper Agent OS.
...
Skipper reads its configuration from a single TOML file:
...
The simplest working configuration only needs an LLM provider API key set as an environment variable. With no config file at all, Skipper boots with Anthropic as the default provider:
...
```toml
# ============================================================
# Skipper Agent OS -- Complete Configuration Reference
# ============================================================
...
# --- Memory ---
[memory]
# sqlite_path = "~/.skipper/data/skipper.db"  # Auto-resolved if omitted
...
```toml
[memory]
# sqlite_path = "/custom/path/skipper.db"
embedding_model = "all-MiniLM-L6-v2"
consolidation_threshold = 10000
decay_rate = 0.1
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sqlite_path` | path or null | `null` | Explicit path to the SQLite database file. When `null`, defaults to `{data_dir}/skipper.db`. |
...
Configures the OFP (Skipper Protocol) peer-to-peer networking layer with HMAC-SHA256 mutual authentication.
...
| `user_id` | string | `""` | Bot user ID (e.g., `"@skipper:matrix.org"`). |
...
| `nick` | string | `"skipper"` | Bot nickname. |
| `password_env` | string or null | `null` | Env var holding the server password (optional). |
| `channels` | list of strings | `[]` | IRC channels to join (e.g., `["#skipper", "#general"]`). |
...
| `nick` | string | `"skipper"` | Bot nickname in Twitch chat. |
| `default_agent` | string or null | `null` | Agent name to route messages to. |
...
| `username` | string | `"skipper"` | Bot username in Mumble. |
...
Agent-to-Agent protocol configuration, enabling inter-agent communication across Skipper instances.
