...
Common issues, diagnostics, and answers to frequently asked questions about Skipper.
...
Skipper uses `tracing` for structured logging. Set the log level via environment:

```bash
RUST_LOG=info skipper start          # Default
RUST_LOG=debug skipper start         # Verbose
RUST_LOG=skipper=debug skipper start  # Only Skipper debug, deps at info
```
...
### `skipper` command not found after install
...
**Fix**: Run `skipper init` to create the default config:
```bash
skipper init
```

This creates `~/.skipper/config.toml` with sensible defaults.
...
```bash
RUST_LOG=skipper_channels=debug skipper start
```
...
**Automatic protection**: Skipper has a built-in loop guard:
...
**Check**:
1. TOML manifest is valid: `skipper agent spawn --dry-run manifest.toml`
...
2. Set the model to `skipper:agent-name` (e.g., `skipper:coder`)
...
3. Try deleting `~/.skipper/daemon.json` and restarting
...
If slower:
- Check database size (`~/.skipper/data/skipper.db`)
...
1. Add the channel config to `~/.skipper/config.toml` under `[channels]`
2. Set the required environment variables (tokens, secrets)
3. Restart the daemon

### How do I update Skipper?
...
Only LLM API calls go to the provider's servers. All agent data, memory, sessions, and configuration are stored locally in SQLite (`~/.skipper/data/skipper.db`). The OFP wire protocol uses HMAC-SHA256 mutual authentication for P2P communication.

### How do I back up my data?

Back up these files:
- `~/.skipper/config.toml` (configuration)
- `~/.skipper/data/skipper.db` (all agent data, memory, sessions)
...
```bash
rm -rf ~/.skipper
skipper init  # Start fresh
```

### Can I run Skipper without an internet connection?
...
### What's the difference between Skipper and OpenClaw?

| Aspect | Skipper | OpenClaw |
|--------|----------|----------|
...
Skipper can import OpenClaw configs: `skipper migrate --from openclaw`
...
```bash
RUST_LOG=skipper_runtime=debug,skipper_channels=info skipper start
```

### Can I use Skipper as a library?

Yes. Each crate is independently usable:
```toml
[dependencies]
skipper-runtime = { path = "crates/skipper-runtime" }
skipper-memory = { path = "crates/skipper-memory" }
```

The `skipper-kernel` crate assembles everything, but you can use individual crates for custom integrations.
