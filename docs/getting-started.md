...
This guide walks you through installing Skipper, configuring your first LLM provider, spawning an agent, and chatting with it.
...
Run the init command to create the `~/.skipper/` directory and a default config file:
...
Skipper needs at least one LLM provider API key. Set it as an environment variable:
...
The default config uses Anthropic. To change the provider, edit `~/.skipper/config.toml`:
...
Skipper ships with 30 agent templates. Spawn the hello-world agent:

```bash
skipper agent spawn agents/hello-world/agent.toml
```
...
```bash
skipper agent spawn my-agent.toml
```
...
```bash
skipper agent chat a1b2c3d4-e5f6-...
```
...
agent> I'm the hello-world agent running on Skipper. I can:
...
```
Starting Skipper daemon...
Skipper daemon running on http://127.0.0.1:4200
...
Now that you have Skipper running:
...
- **Set up workflows**: Chain multiple agents together. Use `skipper workflow create` with a TOML workflow definition.
- **Use MCP**: Connect to external tools via Model Context Protocol. Configure in `config.toml` under `[[mcp_servers]]`.
- **Migrate from OpenClaw**: Run `skipper migrate --from openclaw`. See [MIGRATION.md](../MIGRATION.md).
- **Desktop app**: Run `cargo tauri dev` for a native desktop experience with system tray.
- **Run diagnostics**: `skipper doctor` checks your entire setup.

### Useful Commands Reference

```bash
skipper init                          # Initialize ~/.skipper/
skipper start                         # Start the daemon
skipper status                        # Check daemon status
skipper doctor                        # Run diagnostic checks

skipper agent spawn <manifest.toml>   # Spawn an agent
skipper agent list                    # List all agents
skipper agent chat <id>               # Chat with an agent
skipper agent kill <id>               # Kill an agent

skipper workflow list                 # List workflows
skipper workflow create <file.json>   # Create a workflow
skipper workflow run <id> <input>     # Run a workflow

skipper trigger list                  # List event triggers
skipper trigger create <args>         # Create a trigger
skipper trigger delete <id>           # Delete a trigger
...
skipper channel setup <channel>       # Interactive setup wizard

skipper config show                   # Show current config
skipper config edit                   # Open config in editor

skipper chat [agent]                  # Quick chat (alias)
skipper migrate --from openclaw       # Migrate from OpenClaw
skipper mcp                           # Start MCP server (stdio)
```
