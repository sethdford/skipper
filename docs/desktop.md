...
1. **Tracing init** -- `tracing_subscriber` is configured with `RUST_LOG` env, defaulting to `skipper=info,tauri=info`.
2. **Kernel boot** -- `SkipperKernel::boot(None)` loads the default configuration (from `config.toml` or defaults), wrapped in `Arc`. `set_self_handle()` is called to enable self-referencing kernel operations.
...
4. **Server thread** -- A dedicated OS thread named `"skipper-server"` is spawned. It creates its own `tokio::runtime::Builder::new_multi_thread()` runtime and runs:
   - `kernel.start_background_agents()` -- heartbeat monitor, autonomous agents, etc.
   - `run_embedded_server()` -- builds the axum router via `skipper_api::server::build_router()`, converts the `std::net::TcpListener` to a `tokio::net::TcpListener`, and serves with graceful shutdown.
...
| **Open Config Directory** | Opens `~/.skipper/` in the OS file manager |
| **Quit Skipper** | Logs the quit event and calls `app.exit(0)` |

The tray tooltip reads **"Skipper Agent OS"**.
...
On desktop platforms, `tauri-plugin-single-instance` prevents multiple copies of Skipper from running simultaneously. When a second instance attempts to launch, the existing instance's main window is shown, unminimized, and focused:
...
To actually quit, use the **"Quit Skipper"** option in the system tray menu.
...
Opens a native file picker for `.toml` files. Validates the selected file as an `AgentManifest`, copies it to `~/.skipper/agents/{name}/agent.toml`, and spawns the agent. Returns the agent name on success.

### `import_skill_file`

Opens a native file picker for skill files (`.md`, `.toml`, `.py`, `.js`, `.wasm`). Copies the file to `~/.skipper/skills/` and triggers a hot-reload of the skill registry.

### `get_autostart` / `set_autostart`

Check or toggle whether Skipper launches at OS login. Uses `tauri-plugin-autostart` (launchd on macOS, registry on Windows, systemd on Linux).
...
Opens `~/.skipper/` or `~/.skipper/logs/` in the OS file manager.
...
| Variable | Effect |
|----------|--------|
| `RUST_LOG` | Controls tracing verbosity. Defaults to `skipper=info,tauri=info` if unset. |

All other Skipper environment variables (API keys, configuration) apply as normal since the desktop app boots the same kernel as the headless daemon.
