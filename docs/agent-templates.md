...
Skipper ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
skipper spawn orchestrator
skipper spawn coder
skipper spawn --template agents/writer/agent.toml
```
...
```bash
skipper spawn security-auditor
...
> General-purpose assistant. The default Skipper agent for everyday tasks, questions, and conversations.
...
```bash
skipper spawn customer-support
...
```bash
skipper spawn meeting-assistant
...
```bash
skipper spawn personal-finance
...
skipper spawn coder --name "backend-coder"

# Spawn from a TOML file path
skipper spawn --template agents/custom/my-agent.toml

# List running agents
skipper agents

# Send a message
skipper message <agent-id> "Write a function to parse TOML files"

# Kill an agent
skipper kill <agent-id>
```
