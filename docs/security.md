# Skipper Security Architecture

This document provides a comprehensive technical reference for every security
system in the Skipper Agent Operating System.  All struct names, function
...
Skipper implements **defense-in-depth** security.  No single mechanism is
...
|---|--------|-------|------------------|
| 1 | Capability-Based Security | `skipper-types` | Unauthorized actions by agents |
| 2 | WASM Dual Metering | `skipper-runtime` | Infinite loops, CPU DoS |
| 3 | Merkle Audit Trail | `skipper-runtime` | Tampered audit logs |
| 4 | Taint Tracking | `skipper-types` | Prompt injection, data exfiltration |
| 5 | Ed25519 Manifest Signing | `skipper-types` | Supply chain attacks |
| 6 | SSRF Protection | `skipper-runtime` | Server-Side Request Forgery |
| 7 | Secret Zeroization | `skipper-runtime`, `skipper-channels` | Memory forensics, key leakage |
| 8 | OFP Mutual Auth | `skipper-wire` | Unauthorized peer connections |
| 9 | Security Headers | `skipper-api` | XSS, clickjacking, MIME sniffing |
| 10 | GCRA Rate Limiter | `skipper-api` | API abuse, denial of service |
| 11 | Path Traversal Prevention | `skipper-runtime` | Directory traversal attacks |
| 12 | Subprocess Sandbox | `skipper-runtime` | Secret leakage via child processes |
| 13 | Prompt Injection Scanner | `skipper-skills` | Malicious skill prompts |
| 14 | Loop Guard | `skipper-runtime` | Stuck agent tool loops |
| 15 | Session Repair | `skipper-runtime` | Corrupted LLM conversation history |
| 16 | Health Endpoint Redaction | `skipper-api` | Information leakage |

---

## 2. Capability-Based Security

**Source:** `skipper-types/src/capability.rs`

Skipper uses capability-based security.  An agent can only perform actions
...
**Source:** `skipper-runtime/src/sandbox.rs`
...
**Source:** `skipper-runtime/src/audit.rs`
...
**Source:** `skipper-types/src/taint.rs`

Skipper implements a lattice-based taint propagation model that prevents
...
**Source:** `skipper-types/src/manifest_signing.rs`
...
**Source:** `skipper-runtime/src/host_functions.rs`
...
Skipper uses `Zeroizing<String>` from the `zeroize` crate on every field
...
**LLM Drivers** (`skipper-runtime/src/drivers/`):
...
**Channel Adapters** (`skipper-channels/src/`):
...
**Web Search** (`skipper-runtime/src/web_search.rs`):
...
**Embedding** (`skipper-runtime/src/embedding.rs`):
...
**Source:** `skipper-wire/src/peer.rs`

The Skipper Wire Protocol (OFP) uses HMAC-SHA256 with nonce-based mutual
...
**Source:** `skipper-api/src/middleware.rs`
...
**Source:** `skipper-api/src/rate_limiter.rs`

Skipper uses the Generic Cell Rate Algorithm (GCRA) for cost-aware API
...
**Source:** `skipper-runtime/src/subprocess_sandbox.rs`
...
**Source:** `skipper-skills/src/verify.rs`
...
**Source:** `skipper-runtime/src/loop_guard.rs`
...
**Source:** `skipper-runtime/src/session_repair.rs`
...
**Source:** `skipper-api/src/routes.rs`

Skipper provides two health endpoints with different information levels.
