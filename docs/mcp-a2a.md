...
Skipper implements both the **Model Context Protocol (MCP)** and **Agent-to-Agent (A2A)** protocol, enabling deep interoperability with external tools, IDEs, and other agent frameworks.
...
- [Part 1: MCP (Model Context Protocol)](#part-1-mcp-model-context-protocol)
  ...
  - [MCP Server -- Exposing Skipper via MCP](#mcp-server)
...
The Model Context Protocol (MCP) is a JSON-RPC 2.0 based protocol that standardizes how LLM applications discover and invoke tools. Skipper supports MCP in both directions:

- **As a client**: Skipper connects to external MCP servers (GitHub, filesystem, databases, Puppeteer, etc.) and makes their tools available to all agents.
- **As a server**: Skipper exposes its own agents as MCP tools, so IDEs like Cursor, VS Code, and Claude Desktop can call Skipper agents directly.

Skipper implements MCP protocol version `2024-11-05`.

**Source files:**
- Client: `crates/skipper-runtime/src/mcp.rs`
- Server handler: `crates/skipper-runtime/src/mcp_server.rs`
- CLI server: `crates/skipper-cli/src/mcp.rs`
- Config types: `crates/skipper-types/src/config.rs` (`McpServerConfigEntry`, `McpTransportEntry`)

---

### MCP Client

The MCP client (`McpConnection` in `skipper-runtime`) allows Skipper to connect to any MCP-compatible server and use its tools as if they were built-in.
...
Skipper supports two MCP transports, defined by `McpTransport`:
...
Helper functions (exported from `skipper_runtime::mcp`):
...
Skipper can also act as an MCP server, exposing its agents as callable tools to external MCP clients.

#### How It Works

Each Skipper agent becomes an MCP tool named `skipper_agent_{name}` (with hyphens replaced by underscores). The tool accepts a single `message` string parameter and returns the agent's response.

For example, an agent named `code-reviewer` becomes the MCP tool `skipper_agent_code_reviewer`.

#### CLI: `skipper mcp`

The primary way to run the MCP server is the `skipper mcp` command, which starts a stdio-based MCP server:
...
This command:
1. Checks if an Skipper daemon is running (via `find_daemon()`)
...
- `McpBackend::Daemon` -- forwards requests to a running Skipper daemon via HTTP
- `McpBackend::InProcess` -- boots a full kernel when no daemon is available

#### HTTP MCP Endpoint

Skipper also exposes an MCP endpoint over HTTP at `POST /mcp`. Unlike the stdio server (which only exposes agents), the HTTP endpoint exposes the full tool set (built-in + skills + MCP tools) and executes tools via the kernel's `execute_tool()` pipeline. This means the HTTP MCP endpoint supports:
...
```json
{
  ...
  "result": {
    "protocolVersion": "2024-11-05",
    "capabilities": { "tools": {} },
    "serverInfo": { "name": "skipper", "version": "0.1.0" }
  }
}
```
...
```json
{
  ...
  "method": "tools/call",
  "params": {
    "name": "skipper_agent_code_reviewer",
    "arguments": {
...
After configuration, all Skipper agents appear as tools in the IDE. For example, you can ask Claude Desktop to "use the skipper code-reviewer agent to review this file."
...
Skipper implements A2A in both directions:
...
**Source files:**
- Protocol types and logic: `crates/skipper-runtime/src/a2a.rs`
- API routes: `crates/skipper-api/src/routes.rs`
- Config types: `crates/skipper-types/src/config.rs` (`A2aConfig`, `ExternalAgent`)
...
pub struct AgentCapabilities {
    pub streaming: bool,                 // true -- Skipper supports streaming
...
**AgentSkill** (not the same as Skipper skills -- these are A2A capability descriptors):

```rust
pub struct AgentSkill {
    pub id: String,           // matches the Skipper tool name
...
Agent Cards are built from Skipper agent manifests via `build_agent_card()`. Each tool in the agent's capability list becomes an A2A skill descriptor. Example card:
...
Skipper serves A2A requests through the REST API. The server-side implementation involves:
...
**External Agent Discovery**: The `A2aClient` uses a 30-second timeout and sends a `User-Agent: Skipper/0.1 A2A` header. Failed discoveries are logged but do not block kernel boot.
