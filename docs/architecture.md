...
Skipper is organized as a Cargo workspace with 14 crates (13 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).

```
skipper-cli            CLI interface, daemon auto-detect, MCP server
    |
skipper-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
skipper-api            REST/WS/SSE API server (Axum 0.8), 76 endpoints
    |
skipper-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    |
    +-- skipper-runtime    Agent loop, 3 LLM drivers, 23 tools, WASM sandbox, MCP, A2A
...
skipper-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
skipper-types          Shared types: Agent, Capability, Event, Memory, Message, Tool, Config,
...
When `SkipperKernel::boot_with_config()` is called (either by the daemon or in-process by the CLI/desktop app), the following sequence executes:

```
1. Load configuration
   - Read ~/.skipper/config.toml (or specified path)
   - Apply #[serde(default)] defaults for missing fields
   - Validate config and log warnings (missing API keys, etc.)

2. Create data directory
   - Ensure ~/.skipper/data/ exists

3. Initialize memory substrate
   - Open SQLite database (skipper.db)
...
The memory substrate (`skipper-memory`) provides six layers of storage:
...
The `LlmDriver` trait (`skipper-runtime`) provides a unified interface for all LLM providers:
...
pub trait LlmDriver: Send + Sync {
    ...
    ) -> Result<LlmResponse, SkipperError>;
...
The `ModelCatalog` (`skipper-runtime/src/model_catalog.rs`) provides a registry of all known models, providers, and aliases.
...
Skipper implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
`taint.rs` in `skipper-types` implements taint labels and taint sets. Data from external sources carries taint labels that propagate through operations, enabling information flow analysis.
...
The channel system (`skipper-channels`) provides 40 adapters for messaging platform integration.
...
The skill system (`skipper-skills`) provides 60 bundled skills and supports external skill installation.
...
Skipper implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes Skipper's 23 built-in tools via the MCP protocol. Enables external tools to use Skipper as a tool provider.
...
The Skipper Protocol (OFP) enables peer-to-peer agent communication across machines.
...
The desktop app (`skipper-desktop`) wraps the full Skipper stack in a native Tauri 2.0 application.
...
+-------------------------------------------------------------------+
    ...
    v                        v                   v         v
+------------------+  +--------------+  +--------+  +-----------+
| skipper-runtime |  | skipper-    |  | open-  |  | skipper- |
