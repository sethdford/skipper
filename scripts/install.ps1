# Skipper installer for Windows
# Usage: iwr -useb https://skipper.sh/install.ps1 | iex
#   or:  powershell -c "irm https://skipper.sh/install.ps1 | iex"
#
# Flags (via environment variables):
#   $env:SKIPPER_INSTALL_DIR = custom install directory
#   $env:SKIPPER_VERSION     = specific version tag (e.g. "v0.1.0")

$ErrorActionPreference = 'Stop'

$Repo = "RightNow-AI/skipper"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".skipper\bin"
$InstallDir = if ($env:SKIPPER_INSTALL_DIR) { $env:SKIPPER_INSTALL_DIR } else { $DefaultInstallDir }

function Write-Banner {
    Write-Host ""
    Write-Host "  Skipper Installer" -ForegroundColor Cyan
...
function Get-Architecture {
    ...
    switch ($archUpper) {
        ...
        default {
            ...
            Write-Host "  Try: cargo install --git https://github.com/RightNow-AI/skipper skipper-cli" -ForegroundColor Yellow
            exit 1
        }
    }
}
...
function Get-LatestVersion {
    if ($env:SKIPPER_VERSION) {
        return $env:SKIPPER_VERSION
    }
    ...
    try {
        ...
        Write-Host "    cargo install --git https://github.com/$Repo skipper-cli"
        exit 1
    }
}

function Install-Skipper {
    Write-Banner

    $arch = Get-Architecture
    $version = Get-LatestVersion
    $target = "${arch}-pc-windows-msvc"
    $archive = "skipper-${target}.zip"
    $url = "https://github.com/$Repo/releases/download/$version/$archive"
    $checksumUrl = "$url.sha256"

    Write-Host "  Installing Skipper $version for $target..."
    ...
    # Download to temp
    $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "skipper-install"
    ...
    $exePath = Join-Path $tempDir "skipper.exe"
    if (-not (Test-Path $exePath)) {
        # May be nested in a directory
        $found = Get-ChildItem -Path $tempDir -Filter "skipper.exe" -Recurse | Select-Object -First 1
        if ($found) {
            $exePath = $found.FullName
        }
        else {
            Write-Host "  Could not find skipper.exe in archive." -ForegroundColor Red
            Remove-Item -Recurse -Force $tempDir -ErrorAction SilentlyContinue
            exit 1
        }
    }
    ...
    # Install
    Copy-Item -Path $exePath -Destination (Join-Path $InstallDir "skipper.exe") -Force
    ...
    # Verify
    $installedExe = Join-Path $InstallDir "skipper.exe"
    if (Test-Path $installedExe) {
        try {
            $versionOutput = & $installedExe --version 2>&1
            Write-Host ""
            Write-Host "  Skipper installed successfully! ($versionOutput)" -ForegroundColor Green
        }
        catch {
            Write-Host ""
            Write-Host "  Skipper binary installed to $installedExe" -ForegroundColor Green
        }
    }

    Write-Host ""
    Write-Host "  Get started:" -ForegroundColor Cyan
    Write-Host "    skipper init"
    Write-Host ""
