...
#   docker build -f scripts/docker/install-smoke.Dockerfile \
#     --build-arg SKIPPER_SMOKE_FULL=1 .
...
ARG SKIPPER_SMOKE_FULL=0
RUN if [ "$SKIPPER_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$SKIPPER_SMOKE_FULL" = "1" ] && [ -f "$HOME/.skipper/bin/skipper" ]; then \
        $HOME/.skipper/bin/skipper --version && \
        echo "PASS: skipper binary works"; \
    else \
